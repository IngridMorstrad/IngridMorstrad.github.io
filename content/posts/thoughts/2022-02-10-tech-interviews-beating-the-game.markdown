...
I used to think of behavioral interviews as the easiest rounds. One you didn't really need to prepare for, as could simply bluff your way through it (not lie, really, but let's say you want to be a bit selective with the truth). However, today, I believe you do need to practice for these. One thing I learned in 2021 (thanks to some help), was that all questions in the behavioral round have another question hiding behind them, and *that* is the question you need to answer. This "underlying question" may not be entirely obvious. For example, when asked about a conflict at work, the point is not about the conflict at all, but rather to paint a picture of how diplomatically you handled it. It doesn't matter how poorly the other party in the conflict acted, the question seeks to unearth whether you will be pleasant and amicable under _every_ situation. It isn't lost on me that preparing this way is a little beauty pageant-esque, but it is what it is.

So, how do you get better at behavioral interviews? You could ask friends in non-technical roles for help, especially ones that have conducted or given many such interviews. Or you could practice with your recruiter (a lot of recruiters are open to this, as your performance ties into _their_ performance). For some questions, I prepared a couple of possible answers (using examples at work), and had recruiters help me with which ones to use. Once you have the answers you are going to use for each question, drill them by repeating them to a friend[^3], or even a mirror, until you can say them confidently while having a positive disposition. Interviewers (on account of being human beings), will have unconscious biases, and you want those to fall your way. Confident, well-articulated and positive responses are likelier to get a positive outcome, assuming other factors are similar. While this may seem unsavory, I can't fault this too much, as software engineering *is* a social profession after all. In most cases, you need to work well with others to do your best work[^4].

Behavioral interviews are also a great way to illustrate the kind of scope you had in your role (for example, if your conflict was with a director versus an SDE 2 😉), meaning you improve your interview performance significantly without requiring the time investment other skills (design, coding) do. Probably because more senior roles involve more non-technical skills, the behavioral interview becomes more important the higher you go, so investing time here should pay off in the long run.

### Leetcode
_Time to spend: >= 2 months_  
If you want to clear interviews across the industry, it's no secret that you will have to buck up and go heads down into Leetcode. A good pace to set is to solve around 2 medium-hard difficulty problems per day until you hit ~100-200 medium-hard problems solved (aiming for 200 is safer). The aim here is primarily to familiarize yourself with the set of problems that get asked, and secondarily to get better at solving new, unseen problems. Get Leetcode premium ~1 month before the interview and solve all the questions in the "by company" list for the company you are interviewing for, sorted by frequency (most of these questions should have been done if you've hit 200 problems). Once you've solved all the questions, solve them again. The standard you want to hit is being able to solve a problem in 10 minutes flat, and having it run with no errors on the first try. This means going from seeing the question to coding up the solution and doing a dry-run in 10 minutes. A good tool to aid with this is spaced repetition. Repeatedly solve the problems a few days apart - especially the ones you found hard. At a more granular level, note down common errors that prevented the code from compiling (for example, using Anki - a flashcard system) and drill yourself on those. For example, I'd always forget the order of parameters for "String.join" in Java or whether an array's length was a field or a method (things an IDE usually helps me with). Using Anki sorted this out so well that even if I got to use an IDE in an interview, I wouldn't need to spend any time on it.

| ![Leetcode problems done per day](/assets/images/leetcode_2021_ppd.png) | 
|:--:| 
| *Solve around 2 medium-hard problems per day* |

You'll also want some practice with tackling new problems. This will help you practice *thinking* of solutions from scratch, which should help when you come across an unfamiliar question in an interview. Just knowing that you are capable of tackling *any* problem is a big confidence boost that can improve your interview performance. I'd probably go with a 80-20 split between doing problems for familiarization versus for improving thinking (meaning you spend some more time on it). As interview dates get closer, reduce the time spent on trying to come up with solutions to problems yourself. When doing problems to build familiarization, give yourself 20 minutes at most (the usual length of an interview), and soon start cutting it down to 5. When doing problems to practice thinking, take as much time as you need (especially if the interview is some time away). On occasion I'd sleep on the problem overnight.

If you find that the problems are too hard, try solving some easy problems to build up momentum (spend 20-40 minutes on each problem). If easy problems are still too hard, start reading the solution "actively". Meaning, you stop reading the solution as soon as you think you have gained enough information to make progress. If you get stuck again, read some more of the solution and repeat this as many times as needed. Once you've got an idea of how to solve the problem, it is *very* important to solve it immediately (and then return to it again a few days later, i.e., leverage spaced repetition). A few other tips to keep in mind when solving problems:

- If you come across a problem that uses a common algorithm (like depth-first search or recursion with memoization), and you struggled with it, find a few more problems under that category and solve them.  
- If you find a problem that seems to be a part of a series (usually labeled with a "1" or "2" at the end on Leetcode), solve the entire series.  
- Don't over-optimize! There's no need to get the most-optimal solution. Of course, it is great if you can, but your focus should be getting a working, correct solution first.
- Read the forums to find other solutions that may improve upon your code. However, you may not want to use algorithms with fancy tricks that the interviewer may not understand (provided there are other algorithms that are as optimal).
- Write clean code, and always use proper variable names (no "var x", "var test", etc.). This leaves your code looking more "professional" (hello unconscious bias, my old friend).
- Don't hack your way through the problem. Think of a solution from start-to-end and then begin coding. Getting "stuck" midway through the solution (especially if the solution doesn't work at all) leaves a bad taste. Take a couple of minutes and ensure you really have it.

Many people may think they need to start from scratch by reading an algorithms textbook, but that is the wrong way to go about this. You need practice solving problems, and you'll gain the skills you need to solve more problems by following the approach mentioned here (reading solutions "actively"). However, if you don't have time to do a lot of problems, then fine-tuning/over-fitting your preparation for the company in question is your best bet. 

### Company-specific preparation
_Time to spend: ~2 days (per company)_  
Not frequently listed, but company-specific preparation is probably what gives you the biggest bang for your buck. The best way to go about this, is to go to the Leetcode forums, Glassdoor or Blind, and check for what the company values. Some companies will want you to code bug-free without ever touching the "Run code" button, while others want you to run your code frequently (similar to how you code at work) and use an IDE. Some will want you to walk through the code line-by-line to demonstrate its correctness, while others will want you to write out testcases instead.

In addition, go through questions others have posted, that they may have been asked. Once you find relevant posts, try and solve the problems in code. Do *not* just read the problems and solutions[^5]. Once you identify questions that are likely to be asked, repeatedly solve the questions (as stated in the previous section). For some companies, you can find their entire interview list online! The reason this happens is that companies attempt to calibrate their interview questions across candidates. This usually means that questions that are asked have to be from a limited pool of questions[^6]. If you have completed the previous steps, you should have completed most problems on Leetcode under "by company", meaning this section should take only a couple of days.
...
Lastly, you should plan on doing **nothing** the day before the interview. As a professor once told me, "If you haven't gotten to it by the last day, you're not going to get it on the last day either". You've done enough. Wind down and relax instead. You got this!
