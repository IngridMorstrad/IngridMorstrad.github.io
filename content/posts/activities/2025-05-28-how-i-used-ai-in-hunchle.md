...
I used AI in a big way to build [Hunchle](www.hunchle.com) and I figured it would be useful to share my experience and documenting what coding with AI looks like today.

There were two big cases I used LLMs for in Hunchle, a trivia app:

## Generating questions

In an ideal world, I'd generate 30000 questions by myself, custom-crafted the way I like quiz questions. However, that was more time investment than I'd like, so I tried relying on AI (ChatGPT, Gemini and Grok mostly) to generate some questions for me.

### "General" knowledge
Most of these were run-of-the-mill questions that wouldn't really tickle trivia enthusiasts the right way. Questions like, "What's the capital of Canada?" or "Who wrote Romeo & Juliet?". I decided those would have to do for now. I think this is going to be a common thing with AI, where you trade off getting more optimal results to gain time. TBH, as an industry, software engineers have been creating sub-optimal software (under the direction of "get the next feature out ASAP" & "bug fixing doesn't get you promotions"), so how much worse can things get with AI? I think it's possible it could get MUCH worse. AI can generate code, and therefore bugs, at breakneck speed. With human-written code, the slowness in getting new code out acted (unintentionally) as friction against new bugs being added. With AI, you frequently have 1000 lines of new code, with entire working sections being completely rewritten. This means you are left overwhelmed, and in a worse state than when everyone was manually coding. Interesting times lay ahead.

### Unusable questions
Some other questions were plain useless. Here's an actual example:  
```
Q: This Olympic sport, whose name comes from the French word for 'fencing', involves a foil, épée, or sabre. What is it?
A: Fencing
```

I hope I don't have to explain that one. Here's another one, which I likely will need to explain:  
```
Q: This HBO series, set in a medieval fantasy world, became famous for its shocking deaths and dragons. What is it?
A: Thrones
```

What's up with that? The right answer is "Game of Thrones", but here the AI simply prints "Thrones". So, the instructions I'd given was to prefer last name 1-word answers if possible, but if that wasn't possible, I asked the AI to specify the number of words in the answer. Here, instead of stating "3 words" and leaving the answer as "Game of Thrones", it decided to go for the former, clearly incorrect to a human, approach and simply entered "Thrones".
...
This was a nightmare failure scenario, where the LLM is violating your #1 constraint: don't simply give away the answer! The impact of the error here is not extremely serious, since this is a fun game after all, but imagine if a company used this for something like prompting a user with a hint for their "Security Question". The fallout could bankrupt a company. I tried a few things to get around this. First, I explicitly instructed it, multiple times, not to provide the answer. I realized it was still doing so, for example, in the question abou t Peter Pan & Hook. I realized it may have not realized that a "hook" (object) effectively is the same thing as "Captain Hook" (the character) in this case. So, I further added an instruction not to use the word at all. It would still sometimes slip in the answer, so I added code to search the "hint string" for the occurrence of the answer, and then redact it. This led to couple of other fails where it would present hints that were now leaked because of redaction, as seen here:
...
```
Q: What is the name of the main protagonist of Star Wars: The Return of the Jedi? (2 words)
A: Luke Skywalker
