// To be used on https://myactivity.google.com/
...
times.forEach((timeElement, index) => {
  ...
  let date;
  if (dateText === 'Today') {
    throw new Error('ERROR: Please update the "Today" date in google_activity.js before running this script. Replace the hardcoded date with the current date.');
  } else if (dateText === 'Yesterday') {
    throw new Error('ERROR: Please update the "Yesterday" date in google_activity.js before running this script. Replace the hardcoded date with yesterday\'s date.');
  } else {
    // Format: "November 28"
